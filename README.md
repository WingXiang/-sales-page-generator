...
AI 文案生成使用 **Claude（Anthropic）API**，透過 serverless 代理 [`api/generate.js`](api/generate.js) 呼叫，API key 留在後端。
GitHub Pages 為純靜態主機、無後端，代理（`/api/generate`）無法執行，AI 生成會降級為本地範本；
若要讓線上 AI 真正運作，請部署到 **Vercel**（前端 + serverless 代理一站搞定）。專案已內含 [`vercel.json`](vercel.json)，開箱即用。
...
3. 部署後到 **Settings → Environment Variables** 新增：
   - `ANTHROPIC_API_KEY`：你的 Anthropic API Key（必填，`sk-ant-...`，到 [console.anthropic.com](https://console.anthropic.com/) 取得）
   - `ANTHROPIC_MODEL`：模型 ID（選填，預設 `claude-sonnet-4-6`；更省可改 `claude-haiku-4-5`，更強可改 `claude-opus-4-8`）
...
vercel env add ANTHROPIC_API_KEY production   # 貼上你的 Anthropic Key (sk-ant-...)
vercel --prod            # 正式部署
```
